...
import { ArrowLeft, MessageSquare, KanbanSquare, FolderOpen, Bell, LineChart, GanttChartSquare } from 'lucide-react';
...
export default function ProjectDashboardView({ project, onBack }: { project: Project, onBack: () => void }) {
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden min-h-[600px]">
      <div className="p-4 sm:px-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
         <div className="flex items-center gap-4 border-r border-gray-200 pr-6 mr-2 shrink-0">
           <button onClick={onBack} className="p-2 bg-white border border-gray-200 text-gray-500 rounded-lg hover:bg-gray-50 transition">
             <ArrowLeft className="w-5 h-5"/>
           </button>
           <h2 className="text-xl font-bold text-navy-900 truncate max-w-[150px] sm:max-w-xs">{project.name}</h2>
         </div>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50/30">
         ...
         {activeTab === 'tasks' && <KanbanBoard projectId={project.id} embedded />}
         {activeTab === 'gantt' && (
           <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-4">
         ...
         {activeTab === 'files' && <DocumentsView projectId={project.id} embedded />}
         ...
         {activeTab === 'insights' && (
           <div className="p-4 h-full"><AnalyticsView /></div>
         )}
      </div>
    </div>
  );
